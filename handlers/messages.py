...
def _format_button_label(track: youtube.TrackMeta) -> str:
...
def _build_keyboard(tracks: list[youtube.TrackMeta]) -> InlineKeyboardMarkup:
    rows = [
...
async def search_text(message: Message) -> None:
    ...
    tracks = await youtube.search(query, SEARCH_LIMIT_DM)
    if not tracks:
